...
from src.helpers.activations import Sigmoid, Softmax, ReLU, LeakyReLU
from src.network.network_functions import train, predict
from src.helpers.loss_functions import *

# Load training data
cutoff = 50000  # we load a subset of the full dataset
...
validation_map = mapping_array[
    int(fractions[0] * images_flat.shape[0]) : int(
        (fractions[0] + fractions[1]) * images_flat.shape[0]
    )
]
...
# # Working
# network = [
#     DenseLayer(input_layer_size, 10),
#     Softmax(),
# ]


# Train network
network, training_loss, validation_loss = train(
    network,
    train_x=training_input,
    train_y=training_labels,
    loss_function=CategoricalCrossEntropyLoss,
    valid_x=validation_input,
